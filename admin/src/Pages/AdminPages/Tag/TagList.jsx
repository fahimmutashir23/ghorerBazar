...
import AddCategoryModal from "./AddTagModal";
import UpdateCategoryModal from "./UpdateTagModal";
...
const TagList = () => {
  ...
  const [tag, tagLoading, tagFetch] = useGetTag(page, rows)
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axiosSecure.delete(`/api/delete-tag/${id}`);
      if (res.data) {
  ...
  if (tagLoading || collectionLoading) {
    return <Loader />;
  }
  ...
  return (
    <div className=" rounded-md py-2 px-3">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0  w-full">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0 bg-gray-100 mb-2 w-full ">
          <div className="flex">
            ...
            >
              All( {collectionData.tags} )
            </button>
          </div>
          <AddCategoryModal collectionFetch={collectionFetch} setLoader={setLoader} fetchData={tagFetch} />
        </div>
      </div>
      <div className="overflow-x-auto ">
        <table className="w-full border border-blue-900">
          ...
          <thead className="py-2">
            <tr className="uppercase text-center py-2 bg-gray-700 text-white font-bold ">
              <th className="text-lg border py-1">Category Name</th>
              <th className="text-lg border">Actions</th>
            </tr>
          </thead>
          <tbody className="text-center">
            {tag.result.map((data, idx) => (
              <tr key={idx}>
                <td
        ...
        <UpdateCategoryModal data={data} fetchData={tagFetch} isOpen={isOpen} setIsOpen={setIsOpen} />
