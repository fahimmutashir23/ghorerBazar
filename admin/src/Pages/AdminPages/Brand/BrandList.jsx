  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure(
        `/api/get-brand-list?page=${page}&limit=${rows}`
      );
      return res.data;
    },
  });
    ...
    <div className=" rounded-md py-2 px-3">
      ...
      <div className="overflow-x-auto pb-20">
        <table className="table border border-blue-900">
          ...
          <thead className="h-[40px]">
            <tr className="uppercase text-center h-[40px] bg-gray-700 text-white font-bold ">
              <th className="text-lg border">Brand Name</th>
        ...
        <UpdateBrand
          ...
          setIsOpen={setIsOpen}
        />
        {collectionData.brand > rows && (
          <Paginator
            className="bg-gray-700 max-w-fit mx-auto mt-2 text-white"
            first={first}
            rows={rows}
            totalRecords={collectionData.brand}
