...
const UpdateRole = () => {
  ...
  useEffect(() => {
    if (roleData) {
      const initialPermissions = roleData.permissions
        .filter(item => item)
        .map(item => item);
  ...
  const groupedRoleAccess = roleAccess
    ...
    const handleSubmit = async () => {
        ...
        if(!roleName && !roleData.roleName){
            return toast.error('Role name is required') && setLoader(false)
        } 
        try {
            const res = await axiosSecure.patch(`/api/edit-role/${roleData._id}`, {
              name: roleName || roleData.roleName,
